...
- Перейдите на сайт `<YOUR-SITE-COLLECTION-URL>/SitePages/CustomLearningAdmin.aspx`. При открытии **CustomLearningAdmin.aspx** инициализируется элемент списка **CustomConfig**, который настраивает пути обучения для первого использования. Должна отобразиться страница, которая выглядит следующим образом:
