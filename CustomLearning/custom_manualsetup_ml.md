...
- Вы хотите установить обучающие пути для многоязычной поддержки в одном из сайтов для общения в Организации SharePoint. У сайта есть язык по умолчанию, который не является английским, но является одним из языков, поддерживаемых в пути обучения. Ниже приведены языки, поддерживаемые в пути обучения:
...
Для настройки путей обучения вручную требуется работа с Windows PowerShell и командной консолью SharePoint Online. Ниже приведены действия по настройке путей обучения вручную. 
...
- Необходимо настроить и настроить каталог приложений на уровне клиента. Ознакомьтесь с разделом [Настройка клиента Office 365](https://docs.microsoft.com/sharepoint/dev/spfx/set-up-your-developer-tenant#create-app-catalog-site) и перейдите к разделу Создание сайта каталога приложений. 
- Если каталог приложений на уровне клиента уже подготовлен, вам потребуется доступ к учетной записи, имеющей права на загрузку пакета для выполнения процесса установки. Обычно у этой учетной записи есть роль администратора SharePoint. 
...
>! Ноте Если вам нужно перенести Настраиваемый контент с сайта на новый сайт, ознакомьтесь со статьей [Перенос настраиваемого контента](Migrate custom content). 
